...
def map_depth_map_to_point_clouds(depth_map, mask, mv_mat, K=None, step_size=1, zoom=1.):
    ...
    if K is None:
        ...
        print(f'z min : {vertices[..., 2].min()}, z max : {vertices[..., 2].max()}')
...
def load_normal_map_with_alpha(path):
    image = cv2.cvtColor(cv2.imread(path, cv2.IMREAD_UNCHANGED), cv2.COLOR_RGBA2BGRA)
...
def run_bilateral_integration(
    ...
    normal_map, _ = load_normal_map_with_alpha(normal_path)
