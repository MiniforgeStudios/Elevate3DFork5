...
from FLUX.utils.pca import visualize_pca
...
class FluxSingleAttnProcessor2_0:
    ...
    def __call__(
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.view(batch_size, -1, attn.heads, head_dim).transpose(1, 2)[:, :, :] * 5 + 1
        ...
        # NOTE: Normal Attention
        hidden_states = F.scaled_dot_product_attention(query, key, value, dropout_p=0.0, is_causal=False)

        # NOTE: Extended attention
        # query_inpaint  = query.chunk(query.shape[0])[0]
        # key_extended   = rearrange(key, 'b d c t -> 1 d (b c) t')  
        # value_extended = rearrange(value, 'b d c t -> 1 d (b c) t')
        # hidden_states_ext = F.scaled_dot_product_attention(query_inpaint, key_extended, value_extended, dropout_p=0.0, is_causal=False)
        # hidden_states[0]  = hidden_states_ext
